-- DISPLAY and TABLE library
-- updated: 07-12-2012 added newCircle radius value
 
 
--[[ Display and stage library functions ]]--
 
-- provides a generic reference to the stage display group
stage = display.getCurrentStage()
 
-- screen dimension values
stage, sWidth, sHeight, sCenterX, sCenterY = display.getCurrentStage(),
        display.contentWidth, display.contentHeight, display.contentCenterX, display.contentCenterY
...
--[[
        Description:
                Assigns the radius as a read-only value to the circle.
        
        Params:
                As normal.
        
        Returns:
                A standard circle display object with additional read-only .radius value.
        
        Example:
                local circle = display.newCircle( display.getCurrentStage(), 100, 200, 127 )
                print(circle.radius)
]]--
local newcircle = display.newCircle
display.newCircle = function( ... )
        local circle = newcircle( unpack(arg) )
        if (#arg > 3) then
                circle.radius = arg[4]
        else
                circle.radius = arg[3]
...
-- appends the contents of one table onto the first - do not return anything because the first table is modified
...
--[[
        ...
        Params:
                tbl = source table to copy elements from
                first = first index to take elements from
                last = last index to take elements from
        
        Returns:
                Sub table created from the range of elements from the source table.
]]--
